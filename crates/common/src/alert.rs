...
impl AlertEngine {
    ...
    pub fn evaluate(&self, rule: &AlertRule, node_id: &str, gpu_uuid: &str, value: f64) -> Option<AlertEvent> {
        if !rule.enabled {
            return None;
        }
        ...
        match instance.state {
            ...
            AlertState::Pending => {
                if condition_met {
                    ...
                    let required_count = (rule.duration_seconds + 1) / 2;
...
mod tests {
    ...
    fn test_alert_state_machine() {
        ...
        assert_eq!(engine.get_state(&AlertKey::new("rule-1".to_string(), "node-1".to_string(), "gpu-0".to_string())).unwrap(), AlertState::Firing);
    ...
    fn test_different_gpus() {
        ...
        engine.evaluate(&rule, "node-1", "gpu-0", 90.0);
        assert_eq!(engine.get_state(&AlertKey::new("rule-1".to_string(), "node-1".to_string(), "gpu-0".to_string())).unwrap(), AlertState::Pending);
        
        engine.evaluate(&rule, "node-1", "gpu-1", 90.0);
        assert_eq!(engine.get_state(&AlertKey::new("rule-1".to_string(), "node-1".to_string(), "gpu-1".to_string())).unwrap(), AlertState::Pending);
        
        // gpu-2 was never evaluated, so no alert state is tracked yet
        assert!(engine.get_state(&AlertKey::new("rule-1".to_string(), "node-1".to_string(), "gpu-2".to_string())).is_none());
    }
}
